...
void ftimeout P((void))
{
	...
	elog(pidchild > 0
	     && !kill(pidchild, SIGTERM) ? "terminating" : "was waiting for");
	logqnl(lastexec);
...
static void callnewprog(newname)
...
int pipthrough(line, source, len)
char *line, *source;
const long len;
{
 ...
 pip:		rpipe(pbackfd);
	...
	if (!(pidchild = sfork())) {	/* create a sending procmail */
		...
		; {
			...
			if (pwait
			    && (excode = waitfor(pidfilt)) != EXIT_SUCCESS) {
				pidfilt = 0;
...
long pipin(line, source, len, asgnlastf)
char *const line;
char *source;
long len;
int asgnlastf;
{
	...
	if (!sh) {		/* shortcut builtin commands */
		...
		if (!strcmp(test, line)) {
			if (t1 != Tmnate) {
				...
				if (t2 != Tmnate) {
					t3 = strchr(t2, '\0') + 1;
					if (t3 != Tmnate && !strcmp(t2, "=")
					    && strchr(t3, '\0') == Tmnate - 1) {
						int excode;
						if (verbose) {
							nlog(pbuiltin);
							elog(oquote);
		...
		if (!(pidchild = sfork()))	/* spawn program */
			rclose(PWRO), shutdesc(), getstdin(PRDO),
			    callnewprog(line);
		rclose(PRDO);
		...
		if ((len = dump(PWRO, ft_PIPE, source, len))
		    && (!ignwerr || (len = 0)))
 ...
 builtin:
	...
	{
		return read2blk(mb, filled, &read_read, &read_cleanup,
				&oldfilled);
	}
	...
	size_t max;
	{
		...
		if (!(pidchild = sfork())) {	/* create a sending procmail */
			...
			if (!(pidfilt = sfork()))	/* spawn program/filter */
				rclose(PWRO), rdup(PWRI), rclose(PWRI),
				    getstdin(PRDO), callnewprog(name);
			rclose(PWRI);
			rclose(PRDO);
	...
	void exectrap(tp) const char *const tp;
	{
		...
		if (*tp) {
			...
			if (!forkerr(pidchild, buf)) {
				...
				if ((newret = waitfor(pidchild)) != EXIT_SUCCESS
				    && forceret == -2)
