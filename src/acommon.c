...
#define MAXHOSTNAMELEN	64
	...
	if (!name[0])
		gethostname(name, MAXHOSTNAMELEN), name[MAXHOSTNAMELEN - 1] =
		    '\0';
#else
	static char *name = 0;
	if (!name) {
...
char *dest;			/* convert to a number */
{
	...
	do {
		...
		*dest++ =
		    i + (i < 26 ? 'A' : i < 26 + 26 ? 'a' - 26 : i <
			 26 + 26 + 10 ? '0' - 26 - 26 : i ==
			 26 + 26 + 10 ? '-' - 26 - 26 - 10 : '_' - 26 - 26 -
			 11);
	}
	while (val >>= 6);
	*dest = '\0';
