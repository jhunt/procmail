...
const char shell[] = "SHELL", lockfile[] = "LOCKFILE", newline[] =
    ...
    "Unexpected EOL\n", *const *gargv, *const *restargv =
    &nullp, *sgetcp, pmrc[] = PROCMAILRC, *rcfile, dirsep[] =
    DIRSEP, devnull[] = DevNull, empty[] = "", lgname[] =
    "LOGNAME", executing[] = "Executing", oquote[] = " \"", cquote[] =
    "\"\n", procmailn[] = "procmail", whilstwfor[] =
    " whilst waiting for ", home[] = "HOME", host[] = "HOST", *defdeflock =
    empty, *argv0 = empty, curdir[] = { chCURDIR, '\0' }, slogstr[] =
    "%s \"%s\"", conflicting[] = "Conflicting ", orgmail[] =
    "ORGMAIL", insufprivs[] = "Insufficient privileges\n", defpath[] =
    DEFPATH, exceededlb[] = "Exceeded LINEBUF\n", errwwriting[] =
    "Error while writing to", Version[] = VERSION;
...
static auth_identity *savepass(spass, uid)
...
const char *const argv[];
{
	...
	; {
		...
		if (argc) {	/* sanity check, any argument at all? */
			Deliverymode =
			    !!strncmp(lastdirsep(argv0 = argv[0]), procmailn,
				      STRLEN(procmailn));
			for (argc = 0;
			     (chp2 = (char *)argv[++argc]) && *chp2 == '-';)
 ...
 confldopt:		{
		...
		; {
			...
			checkprivFrom_(euid,
				       passinvk ? auth_username(passinvk) : 0,
				       override);
 ...
 nodevnull:
			{
				writeerr(devnull);
				syslog(LOG_EMERG, slogstr, errwwriting,
				       devnull);
			...
			filled = 0;
			if (!passinvk
			    || !(chp2 = (char *)auth_username(passinvk)))
				chp2 = buf;
#ifdef LMTP
			if (Deliverymode == 2) {
 ...
 dorcpt:			if (enoughprivs
				    (passinvk, euid, egid, auth_whatuid(pass),
				     auth_whatgid(pass)))
					goto Setuser;
				nlog(insufprivs);
				syslog(LOG_CRIT,
				       "Insufficient privileges to deliver to \"%s\"\n",
				       chp2);
 ...
 nospecial:						{
			...
			if (passinvk && idhint) {	/* if same uid as $LOGNAME, use latter */
				auth_identity *idpass =
				    auth_finduser((char *)idhint, 0);
				if (idpass) {
					if (auth_whatuid(passinvk) ==
					    auth_whatuid(idpass))
						pass = idpass;
				}
			}
 ...
 nix_sysmbox:
	...
	while (chp = (char *)argv[argc]) {	/* interpret command line specs first */
		...
		if (!asenvcpy(chp) && mailfilter) {
			...
			for (restargv = argv + argc; restargv[crestarg];
			     crestarg++) ;
			break;
		}
	}
	...
	if (rcfile && !mailfilter) {	/* "procmail rcfile..." */
		...
		if (!rcfile) {	/* no rcfile on the command line */
			...
			rctype =
			    strchr(dirsep,
				   *rcfile) ? rct_ABSOLUTE : rct_CURRENT;
			strcpy(buf, rcfile);
		}
 ...
 suspicious_rc:
		 ...
		 */
	{
		...
		if (((stbuf.st_uid != uid && stbuf.st_uid != ROOT_uid ||	/* check uid, */
		     ...
		     ((stbuf.st_mode & (S_IWOTH | S_IXOTH)) ==
		      (S_IWOTH | S_IXOTH) || rctype == rct_DEFAULT
		      && (stbuf.st_mode & (S_IWGRP | S_IXGRP)) ==
		      (S_IWGRP | S_IXGRP) && (NO_CHECK_stgid
					      || stbuf.st_gid != gid)))) {
			*chp = c;
			goto suspicious_rc;
		}
...
static int mainloop P((void))
{
 ...
 commint:		do
		...
		if (testB(':')) {	/* check for a recipe */
			...
			do {
				...
				i = conditions(flags, prevcond, lastsucc,
					       lastcond, skiprc != 0, nrcond);
				if (!skiprc) {
					if (!flags[ALSO_NEXT_RECIPE]
					    && !flags[ALSO_N_IF_SUCC])
 ...
 progrm:		if (testB('!')) {	/* forward the mail */
				...
				if (*flagsendmail) {
					...
					if (!
					    (q =
					     simplesplit(chp + 1, flagsendmail,
							 buf + linebuf - 1,
							 &got)))
						goto fail;
					*(chp = q) = '\0';
 ...
 nolock:						{
											    ...
											    ('\n')))
				{
					...
					if (flags[PASS_BODY])
						nlog(extrns),
						    elog("deliver-body flag"),
						    elog(ignrd);
					if (flags[PASS_HEAD])
						nlog(extrns),
						    elog("deliver-head flag"),
						    elog(ignrd);
					if (flags[IGNORE_WRITERR])
						nlog(extrns),
						    elog
						    ("ignore-write-error flag"),
						    elog(ignrd);
					if (flags[RAW_NONL])
						nlog(extrns),
						    elog("raw-mode flag"),
 ...
 logsetlsucc:				if (succeed && flags[CONTINUE]
					    && lgabstract == 2)
						logabstract(tgetenv
							    (lastfolder));
 setlsucc:				rawnonl = 0;
