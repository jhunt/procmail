...
static char *skltmark(nl, current)
int nl;
char **current;
{
	char *from = *current, *p;
...
yourself.");
	...
	ps("FW_content",
	   "\"|IFS=' '&&p=@BINDIR@/procmail&&test -f $p&&exec $p -Yf-||\
...
exec $p||exit 75 \2fB#\2fP\2fIYOUR_USERNAME\2fP\"");
	...
	plist("PRESTENV",
	      ...
	      prestenv, ".\1.ad", "", " and ");
	plist("KEEPENV", ", except for the value of ", keepenv, "", "",
	      " and ");
...
#ifdef RESTRICT_EXEC
	ps("RESTRICT_EXEC",
	   "\1.PP\1Users with userids >= @RESTRICT_EXEC_ID@ are\1\
...
An attempt to execute a program from within the rcfile was blocked.");
...
#endif
	ps("LD_ENV_FIX",
	   "\1.PP\1For security reasons, upon startup procmail will\
 ...
 behavior of the runtime linker.");
	...
	ps("ETCRC_desc",
	   etcrc ? "\1.PP\1If no rcfiles and no\1.B \2-@PRESERVOPT@\1have\
...
Care must be taken when creating @ETCRC@, because, if circumstances\
 ...
 @PROCMAILRC@ file of course)." : "");
	ps("ETCRC_files",
	   etcrc ? "\1.TP\1.B @ETCRC@\1initial global rcfile" : "");
 ...
 @ETCRC@ file is executed on behalf of the recipient." : "");
	ps("ETCRC_warn",
	   etcrc ? "\1.PP\1The\1.B @ETCRC@\1file might be executed\
...
\1without backward references (i.e. the parent directory cannot\
 ...
 take on the identity of the owner of the rcfile (or symbolic link).");
	ps("ETCRCS_files",
	   "\1.TP\1.B @ETCRCS@\1special privileges path for rcfiles");
	ps("ETCRCS_warn",
	   "\1.PP\1Keep in mind that if\1.BR chown (1)\1is permitted\
...
a security violation was found (e.g. \1.B \2-@PRESERVOPT@\1or variable\
 ...
 \2fIargument\2fP\" ]\1.B \2-@LMTPOPT@\1");
	ps("LMTPOPTdesc",
	   "\1.TP\1.B \2-@LMTPOPT@\1This turns on LMTP mode, wherein\
