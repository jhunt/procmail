...
static char *bufcur;
static const char lhlomsg[] = "250-localhost\r\n\
250-SIZE\r\n\
250-8BITMIME\r\n\
250-PIPELINING\r\n\
250-CHUNKING\r\n\
...
int flush;
{
	...
	if ((already = bufcur - buf2) + len > linebuf || flush) {
		if (already
		    && already != rwrite(savstdout, bufcur = buf2, already)
...
static char *extractaddress(path)
char *path;
{
...
char *invoker;
{
	static const char cLHLO[] = "LHLO ", cMAIL[] = "MAIL FROM:", cRCPT[] =
	    "RCPT TO:", cDATA[] = "DATA", cBDAT[] = "BDAT", cRSET[] =
		...
		case 'M':
			...
			; {
				...
				if (!(from = slurpaddress())) {
					msg =
					    "553 5.1.7 Unable to parse MAIL address\r\n";
					goto message;
				}
				size = 0;
				goto jumpin;
				do {
					...
					default:
						skiptoeol;
 unknown_param:				msg =
						    "504 5.5.4 unknown MAIL parameter or bad value\r\n";
						goto message;
					}
 jumpin:				do
						c = getL();
 ...
 jumpout:			rpipe(pipefds);
		...
		case 'R':
			...
			if (currcpt == lastrcpt) {	/* do I need some space? */
				...
				if (!(path = slurpaddress())
				    || !(mailbox = extractaddress(path))) {
					msg =
					    "550 5.1.3 address syntax error\r\n";
					goto message;
				}
/* if we were to handle ESMTP params on the RCPT verb, we would do so here */
 ...
 deliver:		readmail(2, 0L);	/* fix up things */
			lastrcpt = rcpts;
			rcpts =
			    realloc(rcpts, (currcpt - rcpts) * sizeof *rcpts);
 ...
 bad_bdat_param:				msg =
				...
				while (length > 0) {
					int i =
					    readLe(themail.p + filled, length);
					if (!i)
						exit(EX_NOINPUT);
					else if (i < 0)
						goto quit;
		...
		case 'V':
			...
			; {
				...
				if (!(path = slurpaddress())
				    || !(mailbox = extractaddress(path))) {
					msg =
					    "501 5.1.3 address syntax error\r\n";
					goto message;
				}
				skiptoeol;
				if (!(temp = auth_finduser(mailbox, 0))) {
...
int retcode;
{
	...
	if (!retcode)
		...
		if (retcode < 0)
			retcode = EX_SOFTWARE;
		if (0 > (retcode -= EX__BASE)
		    || retcode >= (sizeof ret2LMTP / sizeof ret2LMTP[0]))
...
static char *lmtp_read_crnl(char *p, long left, void *statep)
{
	...
	do {
		...
		while (in < last)
			if ((q = memchr(in, '\r', last - in)) ? q > in : !!(q =
									    last))
			{
				if (in != p)
					memmove(p, in, q - in);
