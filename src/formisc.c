...
static const char *skipcomment(start)
const char *start;
{
	for (;;)
		switch (*++start) {
...
const char *Const * const argv;
{
	...
	if (*argv) {		/* do we have to start a program at all? */
		...
		if (lenfileno >= 0) {
			...
			if (val < 0)
				*chp++ = '-';
			ultstr(lenfileno - (val < 0), val < 0 ? -val : val,
			       chp);
			while (*chp == ' ')
				*chp++ = '0';
		}
		...
		; {
			...
			while (children
			       && waitpid((pid_t) - 1, &excode, WNOHANG) > 0)
				if (!WIFSTOPPED(excode)) {	/* collect any zombies */
					children--;
					if ((excode = WIFEXITED(excode) ?
					     WEXITSTATUS(excode) :
					     -WTERMSIG(excode)) != EXIT_SUCCESS)
						retval = excode;
				}	/* reap some children */
			while (childlimit && children >= childlimit
			       || (child = fork()) == -1 && children)
				for (--children;
				     (excode =
				      waitfor((pid_t) 0)) != NO_PROCESS;) {
