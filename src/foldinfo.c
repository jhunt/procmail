...
int rnmbogus(name, stbuf, i, dolog)
 ...
 * paranoid */
static mode_t trymkdir(dir, paranoid, i)
...
static int mkmaildir(buffer, chp, paranoid)
...
const int paranoid;
{
	...
	int i;
	if (paranoid)
		memcpy(buf2, buffer, i = chp - buffer + 1), buf2[i - 1] =
		    *MCDIRSEP_, buf2[i] = '\0';
	return (strcpy(chp, maildirnew), mode =
		trymkdir(buffer, paranoid, i), S_ISDIR(mode))
	    && (strcpy(chp, maildircur), mode =
		trymkdir(buffer, paranoid, i), S_ISDIR(mode))
	    && (strcpy(chp, maildirtmp), mode =
		trymkdir(buffer, paranoid, i), S_ISDIR(mode));
}				/* leave tmp in buf on success */

int foldertype(type, forcedir, modep, paranoid)
int type, forcedir;
mode_t *const modep;
struct stat *const paranoid;
{
	...
	if (!S_ISDIR(mode) || (type == ft_MAILDIR &&
			       (forcedir =
				1, !mkmaildir(buf, chp, paranoid != 0)))) {
...
const int Deliverymode;
{
	...
	if (!stat(buf, &stbuf)) {
		...
		accspooldir = (wwsdir =	/* world writable spool dir? */
			       ((S_IWGRP | S_IXGRP | S_IWOTH | S_IXOTH) & stbuf.
				st_mode) ==
		...
		; {
			...
			if (Deliverymode && (stbuf.st_mode & S_ISUID ||
				...
				if (!(stbuf.st_mode & OVERRIDE_MASK) && stbuf.st_mode & cumask & (accspooldir ? ~(mode_t) 0 : ~(S_IRGRP | S_IWGRP))) {	/* hold back */
					...
					nlog(enfperm);
					logqnl(buf);
					syslog(LOG_NOTICE, slogstr, enfperm,
					       buf);
					setids();
 ...
 fishy:
	...
	if (!S_ISDIR(stbuf.st_mode)) {
		...
		if (!isgrpwrite && !lstat(defdeflock, &stbuf)
