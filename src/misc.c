...
void progerr(line, xitcode, okay)
const char *const line;
int xitcode, okay;
{
	charNUM(num, thepid);
...
char *egrepin(expr, source, len, casesens)
char *expr;
const char *source;
const long len;
int casesens;
...
int enoughprivs(passinvk, euid, egid, uid, gid)
...
const char *newdynstring(adrp, chp)
...
const char *const chp;
{
	...
	size_t len;
	curr =
	    malloc(ioffsetof(struct dynstring, ename[0]) +
		   (len = strlen(chp) + 1));
...
int conditions(flags, prevcond, lastsucc, lastcond, skipping, nrcond)
char flags[];
...
int nrcond;
{
 ...
 noconcat:
	...
	if (flags[ERROR_DO]) {
		i &= prevcond && !lastsucc;
		if (flags[ELSE_DO])
			nlog(conflicting), elog("else-if-flag"),
			    elog(suppressed), flags[ELSE_DO] = 0;
		if (flags[ALSO_N_IF_SUCC])
			nlog(conflicting), elog("also-if-succeeded-flag"),
			    elog(suppressed), flags[ALSO_N_IF_SUCC] = 0;
	}
	if (flags[ELSE_DO])
		i &= !prevcond;
	if (flags[ALSO_N_IF_SUCC])
