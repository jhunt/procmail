...
struct varstr strenstr[] =
    { {"SHELLMETAS", DEFshellmetas}, {"LOCKEXT", DEFlockext},
{"MSGPREFIX", DEFmsgprefix}, {"TRAP", empty},
{"SHELLFLAGS", DEFshellflags}, {"DEFAULT", DEFdefault}, {"SENDMAIL",
							 DEFsendmail},
...
const char lastfolder[] = "LASTFOLDER", maildir[] = "MAILDIR", scomsat[] =
    "COMSAT", offvalue[] = "no";
int didchd;
long Stdfilled;
char *Stdout;
...
static const char slinebuf[] = "LINEBUF", pmoverflow[] =
    "PROCMAIL_OVERFLOW=yes", exitcode[] = "EXITCODE";
static int setxit;

static struct dynstring *myenv;
static char **lastenv;
			      /* smart putenv, the way it was supposed to be */
const char *sputenv(a)
const char *const a;
{
	static int alloced;
	size_t eq, i;
 ...
 wipenv:
	...
	if (alloced)		/* have we ever alloced the environ array before? */
		...
		alloced = 1, environ =
		    tmemmove(malloc(i), environ, i - sizeof *environ);
...
void initdefenv(pass, fallback, do_presets)
auth_identity *pass;
const char *fallback;
int do_presets;
{
...
void allocbuffers(lineb, setenv)
size_t lineb;
int setenv;
{
	if (buf) {
...
const char *const chp;
{
	static const char logfile[] = "LOGFILE", Log[] = "LOG", sdelivered[] =
	    "DELIVERED", includerc[] = "INCLUDERC", eumask[] =
	    "UMASK", dropprivs[] = "DROPPRIVS", shift[] = "SHIFT", switchrc[] =
	    "SWITCHRC";
