...
static long getchunk(s, fromw, len)
const int s;
const char *fromw;
const long len;
{
...
static int dirfile(chp, linkonly, type)
char *const chp;
const int linkonly, type;
{
	...
	if (type == ft_MH) {
		...
		if (dirp = opendir(buf)) {
			while (dp = readdir(dirp))	/* there still are directory entries */
				if ((j = strtol(dp->d_name, &chp2, 10)) > i
				    && !*chp2)
 ...
 exlb:		{
			...
			while ((ok =
				linkonly ? rlink(buf2, buf, 0) : hlink(buf2,
								       buf))
			       && errno == EEXIST);
			if (linkonly) {
				yell(lkingto, buf);
		...
		ultoan((unsigned long)stbuf.st_ino,
		       strcpy(chp, msgprefix) + mpl);
	}
	if (linkonly) {
		yell(lkingto, buf);
 ...
 opn:		unlink(buf2);	/* success; remove the original */
	else if (errno = EEXIST || !stat(buf, &stbuf) || errno != ENOENT
		 || rename(buf2, buf))
...
const int ignwerr, dolock;
{
	...
	if (linkfolder) {	/* any additional directories specified? */
		...
		if (blen = Tmnate - linkfolder)	/* copy the names into safety */
			Tmnate = (linkfolder =
				  tmemmove(malloc(blen), linkfolder,
					   blen)) + blen;
		else
			linkfolder = 0;
	}
 ...
 dumpf:	{
			switch (errno) {
			case ENOSPC:
				nlog("No space left to finish writing"),
				    logqnl(buf);
				break;
#ifdef EDQUOT
			case EDQUOT:
				nlog("Quota exceeded while writing"),
				    logqnl(buf);
				break;
#endif
			default:
			...
			if (lasttell >= 0 && !truncate(boxname, lasttell)
			    && (logopened || verbose))
		...
		*chp++ = *MCDIRSEP_;
		; {
			...
			for (;;) {
				...
				if (0 >
				    (fd =
				     unique(buf, chp, linebuf, NORMperm,
					    verbose, doFD | doMAILDIR)))
					goto nfail;
				if (dump(fd, ft_MAILDIR, source, len)
				    && !ignwerr)
					goto failed;
	...
	; {
		...
		if (rhead == 2)	/* already read, just examine what we have */
			...
			if (tobesent > dfilled && isprivate) {	/* put it in place here */
				tmemmove(themail.p + dfilled, thebody, filled -=
					 tobesent);
				tmemmove(themail.p, new.p, dfilled);
				resizeblock(&themail, filled += dfilled, 1);
				freeblock(&new);
			} else {	/* too big or must share -- switch blocks */
				resizeblock(&new, filled - tobesent + dfilled,
					    0);
				tmemmove(new.p + dfilled, thebody, filled -=
					 tobesent);
				freeblock(&themail);
				themail = new;
				private(1);
 ...
 eofheader:
	...
	if (contlengthoffset) {
		...
		for (actcntlen = places = 0;;) {
			...
			default:	/* fill'r up, please */
				if (places <= sizeof num - 2)
					*chp++ = '9', places++, actcntlen =
					    (unsigned long)actcntlen *10 + 9;
				else
		...
		if (!num[places])	/* does it fit in the existing space? */
			tmemmove(themail.p + contlengthoffset, num, places),
			    actcntlen = cntlen;
