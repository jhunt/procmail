...
struct field **addfield(pointer, text, totlen)
...
register const struct field *p;
{
	if (xheader || Xheader) {	/* extracting only? */
		if (findf(p, &xheader)) {	/* extract field contents */
			char *chp, *echp;
			echp = (chp =
				(char *)p->fld_text + p->id_len) +
			    (int)(p->Tot_len - p->id_len - 1);
			if (zap) {
				chp = skpspace(chp);
				while (chp < echp) {
					switch (*--echp) {
