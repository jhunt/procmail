...
int skipping;
{
	...
	return skipping ? (const char *)0 :	/* speed this up when skipping */
	    ...
	    !j ? argv0 : j <= crestarg ? restargv[j - 1] : (const char *)0;
}
 ...
 */
int readparse(p, fpgetc, sarg, skipping)
register char *p;
int (*const fpgetc) ();
const int sarg;
int skipping;
{
 ...
 ieofstr:			i = '\0';
			...
			case '_':
				startb =
				    incnamed ? incnamed->ename : (char *)empty;
				goto ibreak;
			case '-':
...
char *simplesplit(to, from, fencepost, gotp)
char *to;
