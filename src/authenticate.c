...
static const struct passwd *cgetpwnam(user, sock)
...
static const struct passwd *cgetpwuid(uid, sock)
const uid_t uid;
const int sock;
{
...
/*const*/ auth_identity *auth_finduser(user, sock)
char *const user;
const int sock;
{
...
/*const*/ auth_identity *auth_finduid(uid, sock)
const uid_t uid;
const int sock;
{
...
#ifndef PROCMAIL
int auth_checkpassword(pass, pw, allowemptypw)
...
const char *auth_getsecret(pass)
const auth_identity *const pass;
{
...
void auth_copyid(newpass, oldpass)
...
#ifndef NOpw_class
		if (p->pw_class)
			bbzero(p->pw_class, strlen(p->pw_class));
#endif
#ifndef NOpw_gecos
		if (p->pw_gecos)
...
const char *auth_mailboxname(pass)
auth_identity *const pass;
{
...
#ifdef MAILSPOOLHOME
	{
		...
		if (!
		    (pass->mbox =
		     malloc((i =
			     strlen(pass->pw->pw_dir)) + STRLEN(mailfile) + 1)))
			return "";
...
#else
	{
		...
		if (!
		    (pass->mbox =
		     malloc(STRLEN(mailspooldir) + MAILSPOOLHASH * 2 +
			    strlen(pass->pw->pw_name) + 1 +
			    STRLEN(MAILSPOOLSUFFIX))))
			return "";
		...
		; {
			...
			size_t i;
			int c;
			for (p = pass->mbox + STRLEN(mailspooldir), n =
			     pass->pw->pw_name, i = MAILSPOOLHASH; i--;
			     *p++ = '/') {
				if (*n)
					c = *n++;
				*p++ = c;
...
uid_t auth_whatuid(pass)
const auth_identity *const pass;
{
	return pass->pw->pw_uid;
}

uid_t auth_whatgid(pass)
const auth_identity *const pass;
{
	return pass->pw->pw_gid;
}

const char *auth_homedir(pass)
const auth_identity *const pass;
{
	return pass->pw->pw_dir;
}

const char *auth_shell(pass)
const auth_identity *const pass;
{
	return pass->pw->pw_shell;
}

const char *auth_username(pass)
const auth_identity *const pass;
{
