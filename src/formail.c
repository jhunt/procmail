...
long initfileno;
char ffileno[LEN_FILENO_VAR + 8 * sizeof(initfileno) * 4 / 10 + 1 + 1] =
    DEFfileno;
...
static void renfield(pointer, oldl, newname, newl)
...
static void procfields(sareply)
const int sareply;
{
	...
	while (fldp) {
		...
		if (fp2 = findf(fldp, &Rheader)) {	/* explicitly rename field */
			renfield(afldp, fp2->id_len,
				 (char *)fp2->fld_text + fp2->id_len,
 ...
 fixfldp:
			...
			if ((uf = findf(fldp, &uheader)) && !uf->fld_ref)
				...
				if (fp2->fld_ref) {
					...
					if (afldp ==
					    (ch_afldp =
					     &(*fp2->fld_ref)->fld_next))
						afldp = fp2->fld_ref;	/* deleting own reference */
					for (fldp = Uheader; fldp;
					     fldp = fldp->fld_next)
...
static int digheadr P((void))
{
	...
	while (chp[j - 2] == ' ' || chp[j - 2] == '\t')	/* whitespace before the colon? */
		j--;
	while ((cdigest[i].lnr != j || strncasecmp(cdigest[i].hedr, chp, j - 1))
	       && i--) ;
	return i >= 0 || j > STRLEN(old_)
	    && !strncasecmp(old_, chp, STRLEN(old_)) || j > STRLEN(x_)
...
static char *getsender(namep, fldp, headreply)
char *namep;
struct field *fldp;
const int headreply;
{
	...
	if (i >= 0 && (i != maxindex(sest) || fldp == rdheader)) {	/* found anything? */
		...
		nowm = areply
		    && headreply ? headreply ==
		    1 ? sest[i].wrepl : sest[i].wrrepl : i;
		chp += j;
 ...
 pnewname:			lastm = nowm;
				saddr =
				    strcpy(malloc(strlen(saddr) + 1), saddr);
				if (namep)
					free(namep);
				namep = saddr;
			}
	...
	return namep;
			     ...
			     /* lifted out of main() to reduce main()'s size */
static void elimdups(namep, idcache, maxlen, split)
...
{
	int i, split = 0, force = 0, bogus = 1, every = 0, headreply =
	    0, digest = 0, nowait = 0, keepb = 0, minfields =
	    (char *)progid - (char *)progid, conctenate = 0, babyl =
	    0, babylstart, berkeley = 0, forgetclen;
...
#define Qnext_arg()	if(!*chp&&!(chp=(char*)*++argv))goto usg
 ...
 number:				if (*chp - '0' > (unsigned)9)	/* the number is not >=0 */
				...
				case FM_ReNAME:
					...
					afldp =
					    addfield(lastm ==
						     FM_REN_INSERT ? &iheader :
						     lastm ==
						     FM_DEL_INSERT ? &Iheader :
						     lastm ==
						     FM_ADD_IFNOT ? &aheader :
						     lastm ==
						     FM_ADD_ALWAYS ? &Aheader :
						     lastm ==
						     FM_EXTRACT ? &xheader :
						     lastm ==
						     FM_FIRST_UNIQ ? &uheader :
						     lastm ==
						     FM_LAST_UNIQ ? &Uheader :
						     lastm ==
						     FM_EXTRC_KEEP ? &Xheader :
						     &Rheader, chp, ++lnl);
 ...
 parsedoptions:
	...
	if (babyl) {		/* skip BABYL leader */
		while (getchar() != BABYL_SEP1 || getchar() != BABYL_SEP2
		       || getchar() != '\n')
			while (getchar() != '\n') ;
		while (getchar() != '\n') ;
	}
	...
	if (split) {
		...
		if (!vfileno) {	/* FILENO= found in the environment? */
			...
			tmemmove(ep =
				 malloc(envlen + sizeof *environ), environ,
				 envlen);
			*(vfileno =
			  (char **)((char *)(environ = ep) + envlen)) = 0;
			*--vfileno = ffileno;
		}		/* copy over the array */
	...
	} else if (nrskip > 0 || nrtotal >= 0 || every || digest || minfields
		   || nowait)
		goto usg;	/* only valid in combination with split */
	if ((xheader || Xheader) && logsummary || keepb
	    && !(areply || xheader || Xheader))
 ...
 startover:
	...
	; {
		...
		if (rdheader) {
			...
			if (!strncmp
			    (tmp =
			     (char *)rdheader->fld_text, Article_,
			     STRLEN(Article_)))
				tmp[STRLEN(Article_) - 1] = HEAD_DELIMITER;
				 ...
				 eqFrom_(tmp2 =
					 skpspace(tmp + STRLEN(mailfrom)))) {
				rdheader->id_len = STRLEN(From_);
				tmemmove(tmp, tmp2, rdheader->Tot_len -=
					 tmp2 - tmp);
			}
		}
		namep = 0;
		...
		for (fldp = *(afldp = &rdheader); fldp;) {
			...
			j = fldp->id_len;
			while ((rex[i].lenr != j
				|| strncasecmp(rex[i].headr, chp, j)) && i--) ;
			chp += j;
		...
		if (!forgetclen && (fldp = findf(fcntlength, &rdheader))) {
			*(chp = (char *)fldp->fld_text + fldp->Tot_len - 1) = '\0';	/* terminate it */
			ctlength =
			    strtol((char *)fldp->fld_text + STRLEN(cntlength),
				   (char **)0, 10);
			*chp = '\n';	/* restore the trailing newline */
		}
		tmemmove(parkedbuf = malloc(buffilled), buf, lenparkedbuf =
			 buffilled);
		...
		if (areply) {	/* autoreply requested, we clean up the header */
			...
			if (msid->rexl)	/* do we add an In-Reply-To: field? */
				loadbuf(inreplyto, STRLEN(inreplyto)),
				    loadsaved(msid), addbuf();
			procfields(0);
			...
			if (!findf(fldp, &rdheader))	/* only add what didn't exist */
				if (fldp->id_len + 1 >= fldp->Tot_len &&	/* field name only */
				    (fldp->id_len == STRLEN(messageid) &&
				     !strncasecmp(fldp->fld_text, messageid,
						  STRLEN(messageid))
				     || fldp->id_len == STRLEN(res_messageid)
				     && !strncasecmp(fldp->fld_text,
						     res_messageid,
						     STRLEN(res_messageid))
				    )) {
					char *p;
					const char *name;
					...
					*++p = '\n';
					...
					addfield(&nheader, fldp->fld_text,
						 fldp->Tot_len);
		if (logsummary) {
			...
			if (fldp = findf(fsubject, &rdheader)) {
				concatenate(fldp);
				(chp = fldp->fld_text)[i = fldp->Tot_len - 1] =
				    '\0';
				detab(chp);
				putcs(' ');
				putssn(chp,
				       i >=
				       MAXSUBJECTSHOW ? MAXSUBJECTSHOW : i);
				putcs('\n');
			}
	...
	if (ctlength > 0) {
		if (buffilled)
			lputssn(buf, buffilled), ctlength -=
			    buffilled, buffilled = lnl = 0;
		; {
			int tbl = buflast, lwr = '\n';
 ...
 splitit:				{
				...
				goto fromanyway;
			} else if (split && digest && (lnl || every)
				   && artheadr())
				goto accuhdr;
		}
 ...
 nconlyhead:
	...
	return retval != EXIT_SUCCESS ? retval : split <
	    0 ? EX_IOERR : EXIT_SUCCESS;
}
