...
const char *skipFrom_(startchar, tobesentp)
...
static char *findtstamp(start, end)
...
const char *from, *const invoker;
{
	...
	if (from && !tstamp) {
		...
		fwhom = invoker;
	makeblock(&themail,
		  2 * linebuf + (lfr = strlen(fwhom)) + (linv =
							 strlen(invoker)));
	...
	if (Deliverymode != 2) {	/* if not LMTP */
		...
		if (r > 0 && STRLEN(From_) <= (extra = 1 + rread(	/* is it a From_ line? */
			...
			if (!(rstart = strchr(rstart, '\n'))) {
				...
				extra =
				    rstart ? extra - (++rstart - themail.p) : 0;
			} else {
				...
				if (Deliverymode && privs < 0) {
					if (verbose)	/* discard the bogus From_ */
						nlog(insufprivs);
					syslog(LOG_ERR, slogstr, attemptst,
					       fwhom);
				} else {
					if (tstamp)
						lfr =
						    findtstamp(themail.p +
							       STRLEN(From_),
							       rstart)
						    - themail.p + tstamp;
...
void checkprivFrom_(euid, logname, override)
uid_t euid;
const char *logname;
int override;
{
	...
	if (Deliverymode && *trusted_ids && uid != euid) {
		...
		if (grp = getgrgid(gid))	/* check out the invoker's gid */
			for (logname = grp->gr_name, kp = trusted_ids; *kp;
			     kp++)
