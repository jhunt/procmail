...
const struct eps *const e;
{
	...
	for (;;) {
		...
		case R_0_OR_1:
			r++;
			if (e)	/* first an epsilon, then the rest */
				puteps(rold, r = EOS(r)), pold = r, r =
				    rold + 1, psimp(Ceps pold);
...
static struct eps *maxback(down)
struct eps *down;
{
...
struct eps *bregcomp(a, ign_case)
const char *const a;
...
static struct eps *cleantail(start, thiss, th1)
...
char *bregexec(code, text, str, len, ign_case)
struct eps *code;
...
unsigned ign_case;
{
 ...
 pcstack_switch:;		/* this pc-stack is empty */
	...
	; {
		...
		do {
			...
			reg = initcode;
			for (;;	/* pc-stack */
			     thiss = PC(reg = thiss, th1), PC(reg, th1) =
			     0, start = PCp(reg, th1), reg = reg->next) {
				for (;; reg = stack->next, stack = stack->spawn)	/* pop from work-stack */
					for (;;) {
						switch (reg->opc - OPB) {
						default:
 ...
 checkmatch:
		if (eom) {
			static const char match[] = MATCHVAR, amatch[] =
			    AMATCHVAR;
			char *q;
			if (bom < (char *)text)
				bom = (const char *)text;
			...
			if (getenv(match) == (const char *)text)	/* anal retentive match */
				tmemmove(q = (char *)text, bom, len), q[len] =
				    '\0', bom = q;
			else {
				char *p;
				primeStdout(amatch);
