...
const char *const argv[];
{
	...
	char *cp;
	uid_t uid;
	int sleepsec, retries, invert, force, suspend, retval =
	    EXIT_SUCCESS, virgin = 1;
	static const char usage[] =
	    "Usage: lockfile -v | -nnn | -r nnn | -l nnn \
| -s nnn | -! | -ml | -mu | file ...\n";
		...
		if (*(cp = (char *)*++p) == '-')
			for (cp++;;) {
				...
				case VERSIONOPT:
					...
					elog(auth_mailboxname
					     (auth_finduid(getuid(), 0)));
					elog(lockext);
					elog("\n");
					goto xusg;
				case HELPOPT1:
