...
char **const lockp;
{
	...
	for (lcking |= lck_DELAYSIG;;) {
		...
		case EEXIST:	/* check if it's time for a lock override */
			if (!lstat(name, &stbuf)
			    && stbuf.st_size <= MAX_locksize && locktimeout
			    && !lstat(name, &stbuf)
				 ...
				 */
			{
				...
				if (S_ISDIR(stbuf.st_mode) || unlink(name))
					triedforce =
					    1, nlog("Forced unlock denied on"),
					    logqnl(name);
				else {
...
int xcreat(name, mode, tim, chownit)
...
#ifdef USElockf
		if ((ret |= lockf(fd, F_TLOCK, (off_t) 0))
