	...
	if (!sname) {
		...
		if (!badchars)	/* it's clean, pass it through */
			sname = from;
			 ...
			 (sname = to = malloc(3 * badchars + strlen(from) + 1)))
			sname = "";	/* no memory! */
		else {
			int c;
			while (badchars)
...
int unique(full, p, len, mode, verbos, flags)
char *const full;
char *p;
const size_t len;
const mode_t mode;
 ...
 in:		if (flags & doMAILDIR) {
		...
		*dot = (flags & doMAILDIR) ? '0' + serial : s2c[serial];
		...
		if (i && errno == ENAMETOOLONG) {
			...
			do
				*--op = '\0';	/* try chopping */
			while ((i = lstat(full, &filebuf))
			       && errno == ENAMETOOLONG && op > ldp);
...
#define ropen(path,type,mode)	creat(path,mode)
	...
	while ((!i || errno != ENOENT ||	/* casually check if it already exists */
		(0 > (i = ropen(full, O_WRONLY | O_CREAT | O_EXCL, mode))
		 && errno == EEXIST)) && (i = -1, retry--));
...
#define NEQ(what)	(fdbuf.what!=filebuf.what)	/* and the file? */
		if (lstat(full, &filebuf) || filebuf.st_nlink != 1
		    || filebuf.st_size || NEQ(st_dev) || NEQ(st_ino)
		    || NEQ(st_uid) || NEQ(st_gid) || flags & doCHOWN &&
#endif
		    chown(full, uid, sgid)) {
			rclose(i);
...
const char *const old, *const newn;
{
	...
	if (0 < (ret = rlink(old, newn, &stbuf))) {	/* try a real hardlink */
		...
		if (stbuf.st_nlink < 2 && errno == EXDEV &&	/* try it by conventional means */
		    0 <= (fd =
			  ropen(newn, O_WRONLY | O_CREAT | O_EXCL,
				stbuf.st_mode)))
			return fd + 1;
#endif
		return -1;
